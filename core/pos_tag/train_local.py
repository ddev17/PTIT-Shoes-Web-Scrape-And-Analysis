...
def train(cfg: DictConfig) -> None:
    ...
    corpus = data.DataReader.load_tagged_corpus(
        join(wd, "tmp"), train_file="train.txt", test_file="test.txt"
    )

    train_dataset = corpus.train
    train_dataset = data.preprocess_shoes_data(train_dataset)
    ...
    test_dataset = corpus.test
    test_dataset = data.preprocess_shoes_data(test_dataset)
