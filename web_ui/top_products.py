...
def top_product(text):
    ...
    for token, x in zip(tokens, y):
        if '-PR' in x:
            input_object["price"] = int(token.pop())
        if '-CL' in x:
            input_object["color"] = token.pop()
        if '-SZ' in x:
            input_object["size"] = int(token.pop())
        if '-MT' in x:
